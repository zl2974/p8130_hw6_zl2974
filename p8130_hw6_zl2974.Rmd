...
est_md_all %>%
  broom::tidy() %>%
  filter(!term  %in% c("(Intercept)","ethnicCaucasian")) %>%
  mutate(
  ...
  first()
```
Caucasian group have a positive coefficient of 
`r broom::tidy(est_md_all) %>% filter(term =="ethnicCaucasian") %>% pull(estimate)`
 compared to the African American groups, with a p.value of 
 `r broom::tidy(est_md_all) %>% filter(term =="ethnicCaucasian") %>% pull(p.value)`
